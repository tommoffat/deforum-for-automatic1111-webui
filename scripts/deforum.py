...
def run_deforum(*args, **kwargs):
    ...
    if args_dict['enable_arg_matrix']:
        args_dict_list = deforum_args.process_matrix_args(args_dict)
    else:
        args_dict_list = [{}]

    for j, matrix_overrides in enumerate(args_dict_list):
        for i in range(args_dict['n_batch']):
            ...
            if args_dict['enable_arg_matrix'] and 'checkpoint_schedule' in matrix_overrides:
                # Settings don't include the non-scheduled checkpoint as this is managed by AUTO1111.
                # The only way to vary the checkpoint between runs is to use checkpoint scheduling.
                args_dict['enable_checkpoint_scheduling'] = True

            # For each item in matrix_overrides, override the corresponding value in args_dict
            for key, value in matrix_overrides.items():
                args_dict[key] = value

            # If randomize_animation_prompts is enabled, generate a series of random prompts
            if args_dict['enable_prompt_randomization']:
                args_dict['animation_prompts'] = deforum_args.randomize_animation_prompts(args_dict['animation_prompts'], args_dict['random_prompt_src'])
            ...
            if video_args.skip_video_creation:
                ...
                if video_args.render_steps: # render steps from a single image
                    ...
                    if video_args.aggregate_batch_outputs:
                        mp4_path = os.path.join(args.outdir, 'outputs', f"{args.timestring}_{str(j)}_{i}.mp4")
                        os.makedirs(os.path.split(mp4_path)[0], exist_ok=True)
                    else:
